...
class Checker:
    ...
    def error_s001(self, m, m_line):  # 'Too long'
        if len(m_line) > 79:
            self.errors.append([m, 'S001', self.path, f"{self.path}: Line {m}: S001 {self.codes['S001']}"])

    def error_s002(self, m, m_line):  # Indentation is not a multiple of four
        if len(re.match('^ *', m_line)[0]) % 4 != 0:
            self.errors.append([m, 'S002', self.path, f"{self.path}: Line {m}: S002 {self.codes['S002']}"])
    ...
    def error_s003(self, m, m_line):  # Unnecessary semicolon after a statement
        if ";" in m_line:
            if re.match(".*#.*;.*", m_line):
                ...
                self.errors.append([m, 'S003', self.path, f"{self.path}: Line {m}: S003 {self.codes['S003']}"])
    ...
    def error_s004(self, m, m_line):  # Less than two spaces before inline comments
        if '#' in m_line:
            if re.match("^#.*", m_line):
                ...
                self.errors.append([m, 'S004', self.path, f"{self.path}: Line {m}: S004 {self.codes['S004']}"])

    def error_s005(self, m, m_line):  # TO_DO found (in comments only and case-insensitive)
        if re.match('.*#.*TODO.*', m_line, re.IGNORECASE):
            self.errors.append([m, 'S005', self.path, f"{self.path}: Line {m}: S005 {self.codes['S005']}"])

    def error_s006(self, m, m_line):  # More than two blank lines preceding a code line
        if m_line.strip() == '':
            self.blank_line_count += 1
            if self.blank_line_count > 2:
                self.errors.append([m+1, 'S006', self.path, f"{self.path}: Line {m+1}: S006 {self.codes['S006']}"])
                self.blank_line_count = 0
    ...
    def error_s007(self, m, m_line):  # Too many spaces after construction_name (def or class)
        if re.match(".*(class|def) {2,}.*", m_line):
            ...
            self.errors.append([m, 'S007', self.path, f"{self.path}: Line {m}: S007 {self.codes['S007'].format(name)}"])
        ...
        if 'class' in m_line:
            if not re.match(".* ?class [A-Z][a-zA-Z]*(\\([A-Z][a-zA-Z]*\\))?:", m_line):
                if re.match(".*class [a-z]+", m_line):
                    ...
                    self.errors.append([m, 'S008', self.path, f"{self.path}: Line {m}: S008 {self.codes['S008'].format(class_name)}"])
        ...
        if 'def' in m_line:
            if not re.match(".*def _{,2}?[a-z]*_?[a-z0-9]*_{,2}?\\(.*\\):$", m_line):
                if re.match(".*def +[A-Z]", m_line):
                    ...
                    self.errors.append([m, 'S009', self.path, f"{self.path}: Line {m}: S009 {self.codes['S009'].format(def_name)}"])
                    ...
                    if isinstance(node, ast.FunctionDef):
                        for obj in node.args.args:  # Argument name {} should be written in snake_case
                            ...
                            if not re.match("^[a-z0-9_]*_?[a-z0-9]*$", arg_name):
                                self.errors.append([lineno, 'S010', self.path, f"{self.path.lower()}: Line {lineno}: S010 {self.codes['S010'].format(arg_name)}"])  #
                            ...
                            if isinstance(obj, ast.Assign):
                                for target in obj.targets:
                                    ...
                                    if not re.match("^[a-z0-9_]*\.?_?[a-z0-9]*$", var_name):
                                        self.errors.append([lineno, 'S011', self.path, f"{self.path}: Line {lineno}: S011 {self.codes['S011'].format(var_name)}"])
                        for default in node.args.defaults:  # The default argument value is mutable
                            if not isinstance(default, ast.Constant):
                                ...
                                self.errors.append([lineno, 'S012', self.path, f"{self.path}: Line {lineno}: S012 {self.codes['S012']}"])
